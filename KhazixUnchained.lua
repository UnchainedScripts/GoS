if myHero.charName ~= "Khazix" then PrintChat("Incorrect Champion. Please Select a different script and reload.") return end
...
function KhazixUnchained:Combo(target)
    if target then
        ...
        if(not Utility:IsReady(_Q) and self.Menu.Combo.Assassinate:Value()) then
...
function KhazixUnchained:Draw()
    ...
    if target then 
        ...
        if(calculatedDamage > target.health) then
            Draw.Text("Assassinate Candidate", target.pos:To2D())
        end
    end
    local offOn
    if (self.Menu.Combo.Assassinate:Value()) then offOn = "On" else offOn = "Off" end
    Draw.Text("Assassinate " .. offOn, myHero.pos:To2D() )
end
